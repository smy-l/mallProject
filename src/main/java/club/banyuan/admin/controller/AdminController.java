...
import club.banyuan.mall.mgt.common.Contact;
...
public class AdminController {
  ...
  @ResponseBody
  public Message adminLogin(@RequestBody UmsAdmin adminLogin, HttpSession session) {
    System.out.println(adminLogin.getUsername());
    System.out.println(adminLogin.getPassword());
    UmsAdmin admin = adminService.login(adminLogin);
    session.setAttribute(Contact.ADMIN_SESSION, admin);
...
//    return s;
  ...
  @RequestMapping("/list")
  @ResponseBody
  public Message getAdminList(String keyword, Integer pageSize, Integer pageNum) {
    int total = adminService.getList().size();
    List<UmsAdmin> adminList = adminService.getAdminList(keyword, pageSize, pageNum);
    ...
    data.put("totalPage", (adminList.size() / pageSize) + 1);
    Message message = new Message();
    message.setCode(200);
  ...
  @ResponseBody
  public Message registerAdmin(@RequestBody UmsAdmin umsAdmin) {
    adminService.registerAdmin(umsAdmin);
    Message message = new Message();
    message.setCode(200);
  ...
  @ResponseBody
  public Message updateAdmin(@PathVariable("id") long id, @RequestBody UmsAdmin umsAdmin) {
    System.out.println(id);
    System.out.println(umsAdmin);
    adminService.updateAdmin(id, umsAdmin);
    Message message = new Message();
    message.setCode(200);
